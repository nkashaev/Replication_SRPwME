...
@softscope for ri=1:1000
    ...
    @everywhere const nchain=10000
    @everywhere const repn=(0,nchain)
    # size of chain to use for optimizatio
    nfast=nchain
    ...
    include(rootdir*"/restud_apppower/cudafunctions/cuda_fastoptimv2.jl")
    ...
    include(rootdir*"/restud_apppower/powerfunctions/dgp1.jl")

    # ## Collective
    # include(rootdir*"/restud_apppower/powerfunctions/couples_generator_simulations_2019_10_26.jl")
    ...
    ###############################################################################
    Random.seed!(123*ri)
    #res = bboptimize(objMCcu2; SearchRange = (-pi/2,pi/2), NumDimensions = 4,MaxTime = 100.0, TraceMode=:silent)
...
########
    ...
    CSV.write(dirresults*"/$hypothesis._power_cuda_start.$startit.end.$endit.csv",Resultspower)
    GC.gc()
