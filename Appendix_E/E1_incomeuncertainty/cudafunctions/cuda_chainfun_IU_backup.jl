...
function gchaincu!(d,gamma,cve,rho,chainM=chainM)
    ...
    while r<=repn[2]
      ...
      logtrydens2=(Lambdac[:,2] .-1).^2+(Lambdac[:,3] .-1).^2+(Lambdac[:,4] .-1).^2+((Lambdac[:,4]-Lambdac[:,3]).*Lambdac[:,3] ).^2+((Lambdac[:,3]-Lambdac[:,2]).*Lambdac[:,2] ).^2
      logtrydens3=(Lambda[:,2] .-1).^2+(Lambda[:,3] .-1).^2+(Lambda[:,4] .-1).^2+((Lambda[:,4]-Lambda[:,3]).*Lambda[:,3] ).^2+((Lambda[:,3]-Lambda[:,2]).*Lambda[:,2] ).^2
      ...
      cvesimcu=cu(cvesim)
      if r>0
        chainM[:,:,r]=myfun(d=d,gamma=gamma,Lambda=Lambda,W=W,cve=cve,rho=rho)
