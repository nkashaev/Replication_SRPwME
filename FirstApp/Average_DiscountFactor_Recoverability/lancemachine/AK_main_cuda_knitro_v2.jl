...
include(rootdir*"/restud_apppower/powerfunctions/hyperbolicgenerator_simulations_2019_9_11.jl")

## Collective
#include(rootdir*"/restud_apppower/powerfunctions/couples_generator_simulations_2019_08_02.jl")
...
using KNITRO
model=nothing
#(algorithm=KTR_ACT_LPALG_BARRIER)
model=JuMP.Model(with_optimizer(KNITRO.Optimizer,algorithm=5,ms_enable=0,tuner=1))

JuMP.register(model,:objMCk,4,objMCk,autodiff=true)
@variable(model, -10e300 <= gammav[1:T] <= 10e300)
JuMP.@NLobjective(model,Min,objMCk(gammav[1],gammav[2],gammav[3],gammav[4]))
JuMP.optimize!(model)
